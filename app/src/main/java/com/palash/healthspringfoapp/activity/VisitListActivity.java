...
import com.palash.healthspringfoapp.adapter.VisitListAdapter;
        ...
        switch (item.getItemId()) {
            ...
            case R.id.menu_toolbar_add:
                if (localSetting.checkUnitName(doctorProfileList.get(0).getUnitID())) {
                    localSetting.showWarningAlert(context, context.getResources().getString(R.string.opps_alert), context.getResources().getString(R.string.visit_alert));
                } else {
    ...
    private class GetVisitListTask extends AsyncTask<Void, Void, String> {
        ...
        protected void onPostExecute(String result) {
            if (responseCode == Constants.HTTP_OK_200) {
                ...
                visitListAdapterDB.delete();
                localSetting.showWarningAlert(context, context.getResources().getString(R.string.opps_alert), "Visit not available for selected unit");
            } else {
                localSetting.showErrorAlert(context, context.getResources().getString(R.string.app_name), localSetting.handleError(responseCode));
            }
